...
- ข้อจำกัดที่ยังเหลืออยู่: มีรหัสผ่านแล้วก็จริง แต่ตัว Apps Script Web App เองยังเปิดสาธารณะ (Anyone)
  โดยไม่มี token ตรวจสอบระดับ API — ใครก็ตามที่รู้ URL ยังเรียก action `createUser`/`updateUser` ตรงได้
  โดยไม่ผ่านหน้าเว็บ (เขียนทับรหัสผ่านคนอื่นได้ถ้าจงใจ) เป็นการยอมรับความเสี่ยงแบบเดียวกับที่ Work Permit
  ใช้กับ endpoint สาธารณะสำหรับผู้รับเหมาภายนอกอยู่แล้ว — รหัสผ่านที่เพิ่มรอบนี้ป้องกัน **การสวมรอย
  ผ่านหน้าเว็บโดยเพื่อนร่วมงานทั่วไป** ได้ตามที่ตั้งใจ แต่ไม่ใช่การป้องกันระดับ API ถ้าต้องการเพิ่มอีกชั้น
  ทำได้โดยเพิ่ม secret token ใน request แล้วเช็คใน `doPost()` ก่อนทำงาน (ยังไม่ได้ทำในรอบนี้)
