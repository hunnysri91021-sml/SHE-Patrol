...
- **✅ ทำแล้ว — แผนสำรองข้อมูลเข้า M365**: `backupToMS365Email()` ใน `SHE-Patrol-AppsScript.gs`
  export ข้อมูลทั้งหมดเป็น .xlsx ส่งเข้าอีเมล Outlook ของบริษัทรายสัปดาห์ (ต้องตั้งค่า
  `MS365_Backup_Email` ในแท็บ Settings + time-driven trigger เอง — ดูคอมเมนต์หัวไฟล์ .gs) และมี
  `power-automate/Flow6-Backup-To-OneDrive.md` (standard connector ล้วน ไม่มี premium) คอยเซฟไฟล์
  แนบนั้นเข้า OneDrive/SharePoint ให้อัตโนมัติ — ถ้าบัญชี Google เข้าถึงไม่ได้ในอนาคต ยังมีสำเนาข้อมูล
  อยู่ใน M365 tenant ของบริษัทเอง
