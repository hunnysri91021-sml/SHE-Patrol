...
- ข้อจำกัดที่ยังเหลืออยู่: Apps Script Web App ยังเปิดสาธารณะ (Anyone) อยู่ ป้องกันด้วย
  `API_TOKEN` ที่ต้องตรงกันระหว่าง `APP_CONFIG.API_TOKEN` ใน `index.html` กับ `API_TOKEN` ใน
  `.gs` เท่านั้น (`doPost()` เช็คก่อนทำงานทุก action) — **แต่ token นี้เป็นแค่การกันการเรียก
  ตรงๆ แบบสุ่ม/บอทเก็บข้อมูล ไม่ใช่การป้องกันจริงจัง** เพราะฝัง (hardcode) อยู่ใน `index.html`
  ซึ่งเป็นไฟล์สาธารณะ ใครอ่านโค้ดหน้าเว็บก็เอา token ไปเรียก API เองตรงๆ ได้เหมือนเดิม (เขียนทับ
  ข้อมูลคนอื่นได้ถ้าจงใจ) เป็นการยอมรับความเสี่ยงแบบเดียวกับที่ Work Permit ใช้กับ endpoint สาธารณะ
  สำหรับผู้รับเหมาภายนอกอยู่แล้ว — รหัสผ่าน + token ป้องกัน **การสวมรอยผ่านหน้าเว็บโดยเพื่อนร่วมงาน
  ทั่วไปหรือบอทที่ไม่ได้อ่านโค้ด** ได้ตามที่ตั้งใจ ถ้าต้องการป้องกันระดับ API จริงจัง ต้องทำ
  session token ที่ออกให้ตอน login (เซ็นด้วย secret ฝั่งเซิร์ฟเวอร์ที่ไม่เคยส่งให้ client) แทน — ยังไม่ได้ทำ
...
- การป้องกัน Apps Script Web App แบบจริงจังระดับ session token (เซ็นด้วย secret ฝั่งเซิร์ฟเวอร์
  ที่ไม่ส่งให้ client) — ตอนนี้มีแค่ shared token แบบง่าย (`API_TOKEN`) กันการเรียกตรงๆ แบบสุ่ม/บอท
  เท่านั้น ไม่ใช่การป้องกันระดับ API จริงจัง ดูหัวข้อ "เรื่องผู้ใช้งาน/สิทธิ์" ด้านบน
